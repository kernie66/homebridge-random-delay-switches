...
//    this.emit = switchAccessory.emit
    this.addCharacteristicDelegate({
      key: 'on',
      Characteristic: this.Characteristics.hap.On,
      setter: async (value) => {
        this.switchAccessory.switchOn = value
//        switchAccessory.emit('trigger', value)
        this.log("Calling 'setOn' with value %s", this.switchAccessory.switchOn) 
        this.setOn()
...
//      await this.update()
      ...
      getter: async () => {
        return this.switchAccessory.switchOn
      }
    })
...
//        await this.update
    ...
    this.addCharacteristicDelegate({
      key: 'random',
      value: this.random,
      Characteristic: this.Characteristics.Boolean,
      setter: async (value) => {
        switchAccessory.random = value
//        await this.update
      },
      getter: async () => {
        return switchAccessory.random
      }
    })    
...
//        await this.update
  ...
  async setOn () {
    if (!this.switchAccessory.switchOn) {
      this.log('Stopping the Timer');    
//      clearTimeout(this.timer);
      ac.abort();
      this.switchAccessory.motionTriggered = false;
      if (!this.disableSensor) {
        this.log('Stop any motion')
      }
      ...
      this.log('The delay is %d s', this.timeout)
      setTimeoutPromise(this.timeout, 'foobar', { signal })
      .then( () => {
        this.log('Time is up!');
        this.switchAccessory.switchOn = false;
      });
      .catch((err) => {
...
//          this.switchService.getCharacteristic(Characteristic.On).updateValue(false);
      ...
      if (!this.disableSensor) {
        this.switchAccessory.motionTriggered = true;
        this.log('Triggering Motion Sensor');
        this.switchAccessory.emit('trigger', true)
      }
    }
  }
}
