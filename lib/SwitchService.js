...
class SwitchService extends homebridgeLib.ServiceDelegate {
  constructor (switchAccessory, params = {}) {
    ...
    if (this.cronString) {
      this.cronSchedule = this.getSchedule(this.cronString)
    } else {
      this.cronSchedule = 'Schedule disabled'
    }
    ...
    this.cronJobs = []
    if (this.values.cron) {
      this.cronJobs = this.parseCron(this.values.cron)
    } else if (this.cronString) {
      this.values.cronSchedule = cronstrue.toString(this.cronString)
      this.values.cron = this.cronString
    }
  ...
  stopCronJobs(cronJobs) {
    for (let i in cronJobs) {
      try {
        ...
        this.log('No active cron job, index %d', i)
      }  
    }
  }
  ...
  runCronJob(cronString) {
    ...
    try {
      ...
      for (let i in nextDates) {
        this.log('%s at %s', word, nextDates[i])
        word = 'Then'
      }
    } catch(err) {
