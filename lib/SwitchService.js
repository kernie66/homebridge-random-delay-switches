...
class SwitchService extends homebridgeLib.ServiceDelegate {
  constructor (switchAccessory, params = {}) {
    ...
    let delayValue
    if (newDelay.length === 1) {
      delayValue = parseInt(newDelay[0])
    }
    else {
      delayValue = toSeconds(newDelay.reverse())
    }
    if (delayValue > 863999) {
      delayValue = 863999
      this.warn('Too large delay time, reset to maximum allowed')
    }
    this.log('Delay value: %s', delayValue)

    // config.delay = (delayValue > 863999 ? 863999 : delayValue)

    this.time = toTime(this.delay)
    this.log('Seconds to time: %s = %s days and %s:%s:%s', this.delay, this.time.days, this.time.hours, this.time.minutes, this.time.seconds)
