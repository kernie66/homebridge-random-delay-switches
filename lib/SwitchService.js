...
class SwitchService extends homebridgeLib.ServiceDelegate {
  constructor (switchAccessory, params = {}) {
    ...
    this.addCharacteristicDelegate({
      ...
      this.log("Calling 'setOn' with value %s", switchAccessory.switchOn) 
      this.setOn(switchAccessory)
    }).on('didTouch', (value) => {
      switchAccessory.switchOn = value
      this.log("Repeat 'setOn' with value %s", switchAccessory.switchOn) 
      this.setOn(switchAccessory)
    })
...
//        await this.update
  ...
  async setOn (switchAccessory) {
    if (!switchAccessory.switchOn && !switchAccessory.timeUp) {
      this.log('Stopping the Timer, timeUp = %s', switchAccessory.timeUp)    
      ...
      try {
        ...
        if (switchAccessory.repetition < this.repeats) {
          await wait(5000) // Wait for the motion to complete, 5 s
          this.values.on = true
          switchAccessory.repetition += 1
          this.log('Repetition number: %d of %d', switchAccessory.repetition, this.repeats)
        } else {
          switchAccessory.repetition = 0
        }
      } catch(err) {
        this.log('The timer was aborted')
        switchAccessory.repetition = 0
      }
    }
  }
