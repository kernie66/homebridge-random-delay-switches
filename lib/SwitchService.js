...
class SwitchService extends homebridgeLib.ServiceDelegate {
  constructor(switchAccessory, params = {}) {
    ...
    this.getTimestamp = () => DateTime.now().toUnixInteger()
  ...
  async continueSwitch(switchAccessory) {
    ...
    let delayValue = Math.max(this.values.timestamp - this.getTimestamp(), 5)
  ...
  async setOn(switchAccessory, delayValue) {
    ...
    if (!this.switchOn) {
      ...
      this.values.timestamp = this.getTimestamp() + this.values.timeout
